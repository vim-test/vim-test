...
function! test#java#maventest#build_position(type, position) abort
  ...
  if filename =~# 'IT\|ITCase\|Integration$' && a:type =~# '^nearest\|file$'
    let skip_it_plugins = " -Dsonar.skip=true -Dpit.report.skip=true -Dpit.skip=true -Dpmd.skip=true -Dcheckstyle.skip=true -Ddependency-check.skip=true -Djacoco.skip=true"
